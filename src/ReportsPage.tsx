...
export function ReportsPage({ sidebar, properties }: { sidebar: ReactNode; properties: Array<{ id: string; name: string }> }) {
  ...
  const reportTitle = reportMode === 'DAY' ? `Reporte del ${displayDate(reportDay)}` : 'Reporte por rango de fechas';
  const reportSubtitle = reportMode === 'DAY' ? `Fecha: ${displayDate(reportDay)}` : `Del ${displayDate([reportFrom, reportTo].sort()[0])} al ${displayDate([reportFrom, reportTo].sort()[1])}`;
  ...
  function exportCsv(items = filtered) {
    ...
    const rows = items.map((item) => [dateKey(item.occurredAt), item.propertyName, item.type.name, reportImportanceLabel(item.importance), item.description, item.supervisor.name, item.technician.name, item.requiresInspector ? 'Sí' : 'No', item.inspector?.name ?? 'No aplica', reportStatusLabel(item.status), item.resolution ?? '']);
  ...
  return <div className="page app-page reports-page">{sidebar}
    ...
    <section className="reports-table-card"><div className="reports-section-heading"><div><h2>{quickView==='ALL'?'Report history':quickView==='INSPECTION'?'Reports requiring inspector':`${quickView[0]}${quickView.slice(1).toLowerCase()} reports`}</h2><p>Newest dates first; pending reports appear first within the same day.</p></div><div className="reports-table-actions"><button onClick={()=>exportCsv()}>Download filtered</button></div></div><div className="reports-table-wrap">{loading?<p className="reports-empty">Loading reports…</p>:<table className="reports-table"><thead><tr><th>Date</th><th>Property</th><th>Type</th><th>Importance</th><th>Description</th><th>Inspector</th><th>Technician</th><th>Supervisor</th><th>Status</th><th>Actions</th></tr></thead><tbody>{filtered.length===0&&<tr><td colSpan={10} className="reports-empty">No reports match this selection.</td></tr>}{filtered.map(renderRow)}</tbody></table>}</div></section>
